...
#define Tx_Enable 0
#define Rx_Enable 1
void rs485_ctrl(uint8_t dir);
/* ----------------------- Start implementation -----------------------------*/
void rs485_ctrl(uint8_t dir)
{
	if(dir == Tx_Enable)
	{
		HAL_GPIO_WritePin(DR_uart2_GPIO_Port,DR_uart2_Pin,GPIO_PIN_SET);
	}
	else if (dir == Rx_Enable)
	{
...
vMBPortSerialEnable( BOOL xRxEnable, BOOL xTxEnable )
{
  ...
  */
  ...
  if (xRxEnable) {  
			rs485_ctrl(Rx_Enable);		
    __HAL_UART_ENABLE_IT(&huart2, UART_IT_RXNE);
  } else {    
		rs485_ctrl(Tx_Enable);
