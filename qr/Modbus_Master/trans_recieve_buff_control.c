...
{
  if(HAL_UART_Receive (&huart3 ,(uint8_t *)getbyte,1,0x01) != HAL_OK )
  {
		return HAL_ERROR;
	}
	else
...
{
	HAL_GPIO_WritePin(dr485_GPIO_Port,dr485_Pin,GPIO_PIN_SET);
 if(HAL_UART_Transmit(&huart3 ,(uint8_t *)buf,length,0xff))
 {
   return HAL_ERROR;
 }
 	else
