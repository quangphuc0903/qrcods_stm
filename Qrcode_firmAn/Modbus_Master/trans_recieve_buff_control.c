...
{
  if(HAL_UART_Receive (&huart5 ,(uint8_t *)getbyte,1,0x01) != HAL_OK )
  {
		return HAL_ERROR;
	}
	else
...
{
	HAL_GPIO_WritePin(uart5_dr_GPIO_Port,uart5_dr_Pin,GPIO_PIN_SET);
 if(HAL_UART_Transmit(&huart5 ,(uint8_t *)buf,length,0xff))
 {
   return HAL_ERROR;
 }
 	else
